...
**Changelog (2026-08-05):** an audit of `src/gvls/models/` against the six components above (2026-08-04, `specs/phase5/validation.md` V-0) found three of them effectively inert in the production jet configuration: the variational term contributes 0.25% of the training loss, the inferred latent graph is *complete* on 100% of jets at `M=4` (and its default learner has zero learnable parameters), and the graph-aware prior is never invoked at all. A 24-config sweep confirmed no configuration recovers them. Separately, the full pipeline was measured performing worse than logistic regression on particle count alone (0.6688 vs 0.7552), because fixed-`M` pooling discards multiplicity by construction. Phase 5 (new, `specs/phase5/`) exists to close the gap between what this document claims and what the code does — and it extends component 3: the latent graph is to become genuinely variational (Concrete/Bernoulli edges with their own KL), not merely differentiable, which is the most literal reading of "graph variational latent space" and is currently unimplemented.
