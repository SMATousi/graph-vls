...
**Status: triggered for Cora (2026-07-07) and PubMed (2026-07-08).** Cora: F1 at `d=128, k=20` was 0.8235, and F1 never exceeds 0.828 anywhere in the 36-point grid (flat-and-low pattern). PubMed: F1 at `d=128, k=20` was 0.673 — not a plateau but the *worst* point in the grid, part of a clear monotonic decline as `d` and `k` grow. Two of three datasets now show a decoder-capacity bottleneck (in different ways), which is a stronger signal to implement T3.4 now rather than wait on CiteSeer. Not yet implemented.
