...
- [ ] T3.3 — Rate-distortion sweep: `latent_dim ∈ {4,8,16,32,64,128}` × `k ∈ {1,2,3,5,10,20}`, other hyperparameters fixed to each dataset's Phase 2 NAS-best config; compression-optimal configs written to `configs/compression/{dataset}.yaml`. **Cora done** (2026-07-07) — F1 flat at 0.81–0.83 across the whole grid, fidelity floor not met. **PubMed done** (2026-07-08, remote A100) — F1 *decreases* with more capacity (0.742→0.673 as `d`: 4→128 at `k=20`); PubMed's own NAS-best architecture is the worst point in its own compression grid. Results in `README.md`, `results/compression/{cora,pubmed}.csv`. CiteSeer not started.
- [ ] T3.4 — A_z-conditioned decoder, built only if the existing inner-product decoder proves to be the fidelity bottleneck (trigger: F1 < 0.90 at the largest tested capacity). **Triggered for both Cora and PubMed** — not yet built.
