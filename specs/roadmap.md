...
- [x] ~~T3.4 — A_z-conditioned decoder~~ **Superseded 2026-07-09, revived 2026-07-13.** Triggered for all three datasets (F1 < 0.90 at largest tested capacity everywhere), initially set aside in favor of T3.6. Revived once T3.6 alone didn't close the fidelity gap (the condition the original plan set for revisiting this): `LatentGraphDecoder` (`src/gvls/models/decoder.py`) gives `A_z` a guaranteed path into the reconstruction via one extra unconditional round of message passing, independent of the encoder's `mp_rounds`. Implemented and unit-tested; the production head-to-head sweep (`experiment.decoder=graph_conditioned`) has not been run. See `specs/phase3/validation.md` V-4.
